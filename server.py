import os, json, time, re, threading, logging, sys
...
def delete_calls():
    ...
    global calls
    calls = [c for c in calls if not (c["name"] == to_delete["name"] and c["phone"] == to_delete["phone"])]
    save_data()
...
def vapi_callback():
    ...
    if len(digits) >= 7: phone = digits[-10:]
